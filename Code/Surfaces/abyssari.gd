...
func _ready():
	...
	get_tree().call_group("zombieAbyssari", "queue_free")
	spawnCD.start()
...
func _process(delta):
	...
	$Hud/Radar.hide()
	$Night.color = Color(0,0,0,0.35 * cos((3.14159 / 720) * (Global.minute + (Global.hour * 60))) + 0.55)
...
func _on_zombie_timer_timeout():
	for x in spawnCount:
		if bushes <= 254:
			var zombie = load("res://Scenes/Bushes/zombieAbyssari.tscn").instantiate()
