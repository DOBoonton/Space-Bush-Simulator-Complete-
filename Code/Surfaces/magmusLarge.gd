...
func _ready():
	...
	get_tree().call_group("zombieMagmusLarge", "queue_free")
	spawnCD.start()
...
func _process(delta):
	...
	$Hud/Gear.global_position = Vector2(1115.5,600.25)
	$Hud/Radar.hide()
	$Night.color = Color(0,0,0,0.10 * cos((3.14159 / 720) * (Global.minute + (Global.hour * 60))) + 0.05)
...
func _on_zombie_timer_timeout():
	for x in spawnCount:
		if bushes <= 254:
			var zombie = load("res://Scenes/Bushes/zombieMagmusLarge.tscn").instantiate()
			
			var xpos
			var ypos
			xpos = randf_range(0,72)*16
			ypos = randf_range(14,40)*16
