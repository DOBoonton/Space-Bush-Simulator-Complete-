...
func _ready():
	...
	get_tree().call_group("zombieMagmus", "queue_free")
	spawnCD.start()
...
func _process(delta):
	...
	$Hud/Radar.hide()
	$Night.color = Color(0,0,0,0.15 * cos((3.14159 / 720) * (Global.minute + (Global.hour * 60))) + 0.15)
...
func _on_zombie_timer_timeout():
	for x in spawnCount:
		if bushes <= 254:
			var zombie = load("res://Scenes/Bushes/zombieMagmus.tscn").instantiate()
